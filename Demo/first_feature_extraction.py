...
def extract_region_of_interest_with_mask(image_path, mask, box, output_folder):
    ...
    margins_path = os.path.join(output_folder, "3 Contours.jpg")
    ...
    if len(mask.shape) == 2:
        mask = np.expand_dims(mask, axis=-1)    # For compability add a dimension to the mask


    # Calculate the color histogram of the ROI (excluding black background)
    bgr_rnb = cv2.split(roi)                    # Separate the roi in its three R,G and B planes
    ...
    b_hist = cv2.calcHist(bgr_rnb, [0], mask, [histSize], histRange, accumulate=accumulate)
    g_hist = cv2.calcHist(bgr_rnb, [1], mask, [histSize], histRange, accumulate=accumulate)
    r_hist = cv2.calcHist(bgr_rnb, [2], mask, [histSize], histRange, accumulate=accumulate)   
    
    color_histogram = {
        "blue": b_hist,     "green": g_hist,        "red": r_hist
    }
    ...
    plt.legend()
    histogram_path = os.path.join(output_folder, "5 Color_Histogram.png")
    plt.savefig(histogram_path)
    plt.close()
    print(f"\t- Color histogram plot saved as {histogram_path}")
