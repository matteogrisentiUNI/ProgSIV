...
def H_mask_motion_estimation(previus_frame, next_frame, mask=None, output_folder=None):
    """
    Parameters:
    ...
    Returns:
        ...
        motion: The final homography matrix describe the object motion.
    Workflow: 
        ...
        From the couple of poi derive the object motion with a homography matrixto
    """
    ...
    if len(good_previus_poi) < 4:
        raise ValueError("Not enough valid point correspondences to compute homography.")

    # Compute the homography matrix
    H, _ = cv2.findHomography(good_previus_poi, good_next_poi, method=cv2.RANSAC)
    ...
    return good_previus_poi, good_next_poi, H

def H_motion_estimation(previus_frame, next_frame, previus_poi):
    """
    Parameters:
        previus_frame: first frame
        next_frame: second frame
    ...
    Workflow: 
        ...
        local motion with a homography matrix
    """
    ...
    # Found the velocity of the object
    distances = np.linalg.norm(good_next_poi - good_previus_poi, axis=1)      # compiute the euclidea distance
    mean_speed = np.mean(distances)                                           # compiute the mean velocity of poi  
    #print('mean_spead', mean_speed, "len_poi", len(good_previus_poi)) 

    # If the motion is too fast or there are not enough point to compute homogenity compute the affine or transition 
    if mean_speed > 5 or len(good_previus_poi) < 4:
        if len(good_previus_poi) < 3:     
            transition = np.mean(good_next_poi - good_previus_poi, axis=0)
            tx, ty = transition
            
            H = np.array([[1, 0, tx],       # Create a pure translation matrix (3x3)
                        [0, 1, ty],
                        [0, 0, 1]])    
            #print(f"Warning: motion too fast, mean_spead = {mean_speed:.2f} px")
        else:    
            H_affine, _ = cv2.estimateAffinePartial2D(good_previus_poi, good_next_poi, method=cv2.RANSAC)
            H = np.vstack([H_affine, [0, 0, 1]])   
            # print(f"Warning: motion too fast, mean_spead = {mean_speed:.2f} px")
    else :                               # Compute the homography matrix
        H, _ = cv2.findHomography(good_previus_poi, good_next_poi, method=cv2.RANSAC)
        # H_affine, _ = cv2.estimateAffinePartial2D(good_previus_poi, good_next_poi, method=cv2.RANSAC)
        # H = np.vstack([H_affine, [0, 0, 1]])   
